...
args = train_args()
device = (
    torch.device("cuda", args.gpu) if torch.cuda.is_available() else torch.device("cpu")
)
# initialize visual object classification data
data_dir = args.modelnet40_ft if args.on_dataset == "ModelNet40" else args.ntu2012_ft
adj = None
...
if args.activate_dataset.startswith("coauthorship") or args.activate_dataset.startswith(
    ...
    dataset, idx_train1, idx_test1 = data.load(
        args
    )  # reloading due to different splits.
    idx_val1 = idx_test1
    hypergraph, fts1, lbls1 = (
        dataset["hypergraph"],
        dataset["features"],
        dataset["labels"],
    )
    ...
    fts, lbls, idx_train, idx_test, mvcnn_dist, gvcnn_dist = load_feature_construct_H(
        data_dir,
        gamma=args.gamma,
    ...
    if os.path.exists(f"{data_dir}-Hmatrix"):
        ...
        H = generate_H_from_dist(
            mvcnn_dist=mvcnn_dist,
...
def train_model(
    model, criterion, optimizer, scheduler, num_epochs=25, print_freq=500, args=None
):
    time_start = time.strftime("%Y_%m_%d_%H_%M_%S", time.localtime())
    if args.tensorboard:
        from torch.utils.tensorboard import SummaryWriter

        writer = SummaryWriter(os.path.join(args.save_dir, f"{time_start}"))
    since = time.time()
    ...
    G, H, fts1, lbls1, idx_train1, idx_test1, idx_val1 = get_data(
    ...
    try:
        ...
        B = create_from_coo(
            src, dst, torch.ones_like(src).to(torch.float32), shape=B.shape
        )
        H = B
        print("Converting")
    except:
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        for phase in ["train", "val"]:
            ...
            if args.activate_dataset.startswith("ml"):
                ...
                running_corrects += torch.sqrt(
                    criterion(outputs[idx].squeeze(), lbls1[idx])
                )
        ...
        if epoch % print_freq == 0:
            print(
                f"Best val Acc: {best_acc:4f} at epoch: {best_epoch}, n_layers: {args.nbaseblocklayer} "
            )
            print("-" * 20)
    ...
    _, preds = torch.max(outputs, 1)
    test_acc = torch.sum(preds[idx_test1] == lbls.data[idx_test1]).double() / len(
        idx_test1
    )
    # print(args)
    print(
...
def _main():
    ...
    best_epoch, test_acc, time_elapsed = train_model(
        model,
        criterion,
        optimizer,
        scheduler,
...
if __name__ == "__main__":
    elapsed_times = []
    if args.activate_dataset.startswith("coauthor") or args.activate_dataset.startswith(
        "cocitation"
    ):
